...
from Maja (2024) was only taken from areas strictly labelled with "colony".
...
ggplot() +
  ...
  geom_sf(data = seal_dead_sf, aes(color = "Carcass Observations"), size = 2) +
  geom_sf(data = colony_sf, aes(color = "Seal Colonies"), shape = 17, 
          size = 3) + # Adding the colonies
  scale_color_manual(values = c("red", "blue")) +
  labs(title = "Cape Fur Seal (Arctocephalus pusillus) 
       Carcass Sightings in South Africa") +
  theme_minimal()
...
and extend into Namibia, the same with the seal colonies so we'll need to get 
rid of those. To do this, we can create a spatial filter.

```{r spatial filter}
filter_south_africa_sf <- function(seal_dead_sf, colony_sf) {
  ...
  if (!inherits(colony_sf, "sf")) {
    stop("Error: colony_sf must be an sf object with point geometries.")
  }

  # Filter points that fall inside South Africa for both datasets
  filtered_seal_dead_sf <- st_intersection(seal_dead_sf, south_africa)
  filtered_colony_sf <- st_intersection(colony_sf, south_africa)

  # Return filtered datasets as a list of sf objects
  return(list(filtered_seal_dead_sf = filtered_seal_dead_sf, 
              filtered_colony_sf = filtered_colony_sf))
}

# Apply function to filter both seal_dead_sf and colony_sf
filtered_data <- filter_south_africa_sf(seal_dead_sf, colony_sf)

# Access filtered datasets
seal_dead_sa <- filtered_data$filtered_seal_dead_sf
colony_sa <- filtered_data$filtered_colony_sf
...
filter_south_africa_sf <- function(seal_dead_sf, colony_sf) {
  ...
  # Filter points that fall within South Africa for both datasets using st_within
  seal_dead_within_sa <- st_within(seal_dead_sf, south_africa)
  colony_within_sa <- st_within(colony_sf, south_africa)

  # Convert list results from st_within to logical vectors (TRUE if within, FALSE otherwise)
  seal_dead_within_sa_logical <- lengths(seal_dead_within_sa) > 0
  colony_within_sa_logical <- lengths(colony_within_sa) > 0

  # Subset based on the logical vectors
  filtered_seal_dead_sf <- seal_dead_sf[seal_dead_within_sa_logical, ]
  filtered_colony_sf <- colony_sf[colony_within_sa_logical, ]

  # Return filtered datasets as a list of sf objects
  return(list(filtered_seal_dead_sf = filtered_seal_dead_sf, filtered_colony_sf = filtered_colony_sf))
}
...
ggplot() +
  geom_sf(data = south_africa, fill = "lightgray", color = "black") +
  geom_sf(data = seal_dead_sa, aes(color = "Carcass Observations"), size = 2) +
   geom_sf(data = colony_sa, aes(color = "Seal Colonies"), shape = 17, 
           size = 3) +
  scale_color_manual(values = c("red", "blue")) +
  labs(title = "Cape Fur Seal (Arctocephalus pusillus) 
       Carcass Sightings in South Africa") +
...
# I'll save the colony_sa data as a .csv too since this'll only contain the 
...
leaflet() %>%
  ...
  addLegend(position = "bottomright", colors = "red", labels = "Carcass observations") %>%
